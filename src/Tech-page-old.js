...
var techContents = [
  {
    slide: {
      image: techimage,
      head: "Speciale Invest",
      text:
        "Launches should happen where the customer wants them from or where the mission demands it from. Go ahead & choose your location here.",
    },
    modal: {
      head: "Speciale Invest",
      text:
        "Launches should happen where the customer wants \
         them from or where the mission demands it from.Go ahead & choose your \
          location here.Launches should happen where the customer wants them from \
           or where the mission demands it from.Go ahead & choose your location here. \
           Launches should happen where the customer wants them from or where the mission \
           demands it from.Go ahead & choose your location here.Launches should happen  \
           where the customer wants them from or where the mission demands it from.\
           Go ahead & choose your location here.",
      timeline: [
        {
          image: techimage,
          text: "Launches should happen where the customer wants ",
          year: "2016",
        },
        {
          image: techimage,
...
class TechModal extends Component {
  constructor(props) {
    ...
    this.getCurrentTech = props.getCurrentTech;
  }


  render() {
    if (this.getCurrentTech() < 0)
      return null;
    var currTech = techContents[this.getCurrentTech()];
    console.log(currTech.modal);
    var timelines = currTech.modal.timeline.map((event) => {
      return (
        <div className="tech-timeline-section">
          <img className="tech-timeline-image" src={event.image}>
          </img>
          <div className="tech-timeline-year">
            <p>{event.year}</p>
          </div>
          <p className="tech-timeline-text">
            {event.text}
          </p>
        </div>
      );
    ...
    return (
      <div className="tech-modal-out">
        <div className="tech-modal-outer">
          <div className="tech-modal">
            <p className="tech-close-icon" onClick={(e) => this.closeModal()}>
              X
            </p>
            <div className="tech-modal-section">
              <p className="tech-modal-heading">{currTech.modal.head}</p>
              <p className="tech-modal-text">{currTech.modal.text}</p>
            </div>
...
class Tech extends Component {
  ...
  constructor(props) {
    ...
    this.getCurrentTech = this.getCurrentTech.bind(this);
  } 

  openModal(currTech) {
    if (currTech >= techContents.length || currTech < -1)
      currTech = -1;
     this.setState({
       currentTech: currTech
     }, function () {
  ...
  getCurrentTech() {
    return this.state.currentTech;
  }
  render() {
    Init(); // function to initialize animations and swipers

    var Slides = techContents.map((techContent) => {
       return (
         <div className="tech-swiper-slide swiper-slide">
           <div
             className="tech-slide-content"
             key={techContents.indexOf(techContent)}
           >
             <img className="tech-slide-img" src={techContent.slide.image} />
             <p className="tech-slide-head">{techContent.slide.head}</p>
             <p className="tech-slide-text">{techContent.slide.text}</p>
             <p
               className="tech-slide-link"
               onClick={(e) => this.openModal(techContents.indexOf(techContent))}
             >
               READ MORE
             </p>
           </div>
    ...
    return (
      <div className="body">
        ...
        <TechModal closeModal={this.closeModal} getCurrentTech={this.getCurrentTech}></TechModal>
