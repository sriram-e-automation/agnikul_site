  ...
  function setInitial() {
    ...
    $(".holder").each(function(index) {
        $(this).on("load", function() {
            imagesLoaded++;
            if (imagesLoaded === numberOfMembers) {
                // All images are loaded, start looping
                loop();
            }
        }).attr("src", process.env.PUBLIC_URL + '/images/team-photos/' + (index + 1) + '.png');
    });
}
